...
It's possible to predict with very high accuracy the quality of weight lifting exercises from data obtained from devices weared by the weightlifter. Using the data-set provided by [Velloso, E. et al.(2013)](http://groupware.les.inf.puc-rio.br/har) and a Random Forest model [Breiman, L. (2001)](https://www.stat.berkeley.edu/~breiman/randomforest2001.pdf) I could predict with 99.2% accuracy the quality of the workout.
...
One neat advantage of the Random Forest algorithm is that _an extra cross-validation process to report the out of sample error is not needed_ as the model does it by itself during the building process. In the random forest jargon it's called oob (out-of-bag) error. Its author, in [Breiman](https://www.stat.berkeley.edu/~breiman/RandomForests/cc_home.htm#ooberr), details how its obtained and why an extra cross-validation is not needed.
